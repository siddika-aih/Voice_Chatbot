...
# main.py - Complete ultra-fast voice bot with streaming
import asyncio
import sounddevice as sd
import numpy as np
from google import genai
...
# Multi-key rotation for 503 errors
GEMINI_KEYS = [
...
def get_client():
    ...
    return genai.Client(api_key=GEMINI_KEYS[current_key_index % len(GEMINI_KEYS)])
...
class StreamingVoiceBot:
    ...
    async def transcribe_streaming(self, audio_data):
        """Fast transcription with retry on 503"""
        global current_key_index
        ...
        for attempt in range(max_retries):
            try:
                ...
                response = client.models.generate_content(
                    ...
                    contents=["Transcribe concisely:", uploaded]
                )
                ...
                if "503" in error_msg or "UNAVAILABLE" in error_msg or "overloaded" in error_msg:
                    print(f"⚠️ API overloaded, switching key (attempt {attempt+1}/{max_retries})")
                    current_key_index += 1
        ...
        os.unlink(temp_path)
        raise Exception("All API keys exhausted - Gemini overloaded")
    
    async def generate_response_streaming(self, query, callback=None):
        """
        Generate response with streaming
        callback: async function(chunk) - called for each text chunk
        """
        from data_store import hybrid_retrieve
        
        # Fast retrieval
        print("🤔 Processing...")
        retrieval_task = asyncio.create_task(hybrid_retrieve(query, top_k=3))
        ...
        # Prompt
        prompt = f"""You are a helpful DCB Bank assistant. Answer in 2-3 sentences maximum.

Context: {context[:600]}

History: {history_text[:150]}
            ...
            try:
                ...
                for chunk in response_stream:
                    if chunk.text:
                        ...
                        # Call callback for streaming TTS
                        if callback:
                ...
                error_msg = str(e)
                if "503" in error_msg or "UNAVAILABLE" in error_msg or "overloaded" in error_msg:
                    print(f"⚠️ Switching API key (attempt {attempt+1}/{max_retries})")
                    current_key_index += 1
                    await asyncio.sleep(0.3)
                    continue
                else:
...
if __name__ == "__main__":
    print("Run this via output_data.py server")
